          ...
          <nav className="hdr__nav" aria-label="Primary">
            {NAV_LINKS.map(({ label, href }) => (
              <a
                key={href}
                href={href}
                className={`hdr__nav-link${activeId === href.slice(1) ? ' hdr__nav-link--active' : ''}`}
          ...
          <nav className="hdr__drawer-nav">
            {NAV_LINKS.map(({ label, href }) => (
              <a
                key={href}
                href={href}
                className={`hdr__drawer-link${activeId === href.slice(1) ? ' hdr__drawer-link--active' : ''}`}
                onClick={e => scrollTo(e, href)}
              >
                {label}
...
.hdr__logo-wrap {
  ...
  background: linear-gradient(145deg,#1a1400,#2e2200);
...
.hdr__logo-img { width: 26px; height: 26px; object-fit: contain; }
.hdr__wordmark { display: flex; flex-direction: column; gap: 2px; }
.hdr__wordmark-row { display: flex; align-items: baseline; gap: 6px; }
.hdr__name {
  font-family: 'DM Serif Display', serif;
  font-size: 1.15rem; color: #f0e8d4;
...
.hdr__badge {
  ...
  border: 1px solid rgba(201,168,76,0.28); border-radius: 4px;
  padding: 1px 5px; text-transform: uppercase; line-height: 1.7;
}
.hdr__sub {
  font-family: 'Instrument Sans','DM Sans',sans-serif;
...
.hdr__nav-link {
  position: relative;
  font-family: 'Instrument Sans','DM Sans',sans-serif;
  font-size: 0.84rem; font-weight: 500; color: #6b8299;
  text-decoration: none; padding: 6px 13px; border-radius: 8px;
  letter-spacing: 0.01em; white-space: nowrap;
  transition: color 0.15s, background 0.15s;
}
.hdr__nav-link:hover { color: #f0e8d4; background: rgba(255,255,255,0.05); }
.hdr__nav-link--active { color: #f0e8d4 !important; }
.hdr__nav-link--active::after {
  content: '';
...
.hdr__cta {
  ...
  border: none; border-radius: 9px; color: #0c1018;
  font-family: 'Instrument Sans','DM Sans',sans-serif;
...
.hdr__burger span {
  display: block; height: 1.5px; border-radius: 2px; background: #8fa3b8;
...
.hdr__drawer--open { max-height: 340px; border-top-color: rgba(255,255,255,0.06); }
.hdr__drawer-nav { display: flex; flex-direction: column; padding: 8px 20px; }
.hdr__drawer-link {
  display: flex; align-items: center; justify-content: space-between;
  padding: 13px 8px;
  font-family: 'Instrument Sans','DM Sans',sans-serif;
  font-size: 0.92rem; font-weight: 500; color: #8fa3b8;
...
.hdr__drawer-link--active { color: #f0e8d4; }
.hdr__drawer-link--active { font-weight: 600; }
.hdr__drawer-cta {
  margin: 8px 20px 20px; padding: 13px;
  background: linear-gradient(135deg,#c9a84c,#b8903a);
  border: none; border-radius: 10px; color: #0c1018;
  font-family: 'Instrument Sans','DM Sans',sans-serif;
  font-size: 0.9rem; font-weight: 700; letter-spacing: 0.02em;
  cursor: pointer; box-shadow: 0 2px 16px rgba(201,168,76,0.25);
